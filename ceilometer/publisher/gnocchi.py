...
class GnocchiPublisher(publisher.ConfigPublisherBase):
    ...
    def publish_samples(self, data):
        ...
        LOG.warning("KAG: publish samples")
        ...
        for info in gnocchi_data.values():
            ...
            try:
                ...
                LOG.error(six.text_type(e), exc_info=True)
        LOG.warning("KAG: publish samples done")
    ...
    def batch_measures(self, measures, resource_infos):
        ...
        try:
            ...
            LOG.warning("KAG: retrying posting resource metrics to gnocchi: %s",
                        str(measures))
            self._gnocchi.metric.batch_resources_metrics_measures(
                measures, create_metrics=True)
            LOG.warning("KAG: posted")
    ...
    def _update_resource(self, resource_type, resource, resource_extra):
        LOG.warning("KAG: updating resource %s, %s",
                    str(resource_type), str(resource))
    ...
    def publish_events(self, events):
        LOG.warning("KAG: publish events")
        for event in events:
        ...
        LOG.warning("KAG: publish events done")
